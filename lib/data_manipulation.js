define(module, (exports, require) => {
  ...
  qp.make(exports, {
    ...
    query: function(cmd, values, handler) {
      return this.connection.query(qp.is_array(cmd) ? qp.build(cmd) : cmd, values, handler);
    },
    ...
    begin_transaction: function(config) {
      config.type = 'transaction';
      config.text = 'BEGIN TRANSACTION';
      this.execute(config);
    },

    rollback_transaction: function(config) {
      config.type = 'transaction';
      config.text = 'ROLLBACK TRANSACTION';
      this.execute(config);
    },

    commit_transaction: function(config) {
      config.type = 'transaction';
